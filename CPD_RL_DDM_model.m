...
for trial = 1:length(trials)
    ...
    for t = 1:max_trial_length
        ...
        if settings.sim
            ...
            new_row = {current_trial.trial_number(1), current_trial.trial_type{1}, 8, NaN, '0', num2str(patch_action), NaN, '', 0};
        ...
        if t == 1
            ...
            if settings.sim
                ...
                [simmed_rt, accepted_first_dot_motion] = simulate_DDM(drift, decision_thresh, nondecision_time, starting_bias, 1, .001, max_rt);
                % accepted dot motion
                if accepted_first_dot_motion
                    current_trial.result(2) = patch_action == correct_choice; % result column is 1 if accepted correct dot motion
                    current_trial.accepted_dot_motion(2) = 1;
                else
               ...
               % accepting dot motion, but remember that WFPT always
               % calculates probability of hitting lower boundary
                if current_trial.accepted_dot_motion(2) % chose to accept first dot motion
                    drift = drift * -1;
            ...
            if settings.sim
                % accepting dot motion
                [simmed_rt, accepted_second_dot_motion] = simulate_DDM(drift, decision_thresh, nondecision_time, starting_bias, 1, .001, max_rt);
                % accepted dot motion
                if accepted_second_dot_motion
                    current_trial.result(3) = patch_action == correct_choice; % result column is 1 if accepted correct dot motion
                    current_trial.accepted_dot_motion(3) = 1;
                else
               ...
               % negative drift and lower bias entail greater probability of
               % accepting dot motion
                if  current_trial.accepted_dot_motion(3) % chose to accept dot motion
                    drift = drift * -1;
                    starting_bias = 1 - starting_bias;
                end
                dot_motion_rt_pdf(t,trial) = wfpt(dot_motion_rt - nondecision_time, drift, decision_thresh, starting_bias);
